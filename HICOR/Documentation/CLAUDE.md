...
| File | Responsibility |
|---|---|
| `App/HICORApp.swift` | `@main` entry, SwiftData `ModelContainer`, lens inventory bootstrap |
| `App/ContentView.swift` | Root SwiftUI view (Phase 1: placeholder; later phases: navigation) |
...
| `Services/PersistenceService.swift` | SwiftData `ModelContainer` + insert/fetch CRUD |
| `Services/LensInventoryService.swift` | Loads `DefaultInventory.json`, supports user override in Documents |
| `Services/CloudKitService.swift` | `CKRecord` conversion (real); network ops stubbed in Phase 1 |
...
| 2 | iCloud container provisioning, real CloudKit calls, sync logic | Pending |
| 3 | Camera capture screen (2-5 photos) | Pending |
...
| 7 | Result screen, history/lookup screen | Pending |
| 8 | Session setup + patient entry screens | Pending |
...
- **Container:** `iCloud.com.creativearchives.hicor` (public DB).
- **Capability:** *not yet enabled* in `project.yml`. Phase 2 enables `com.apple.developer.icloud-services` and `com.apple.developer.icloud-container-identifiers` once the container is provisioned in the Apple Developer portal.
