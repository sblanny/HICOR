...
| `Services/OCR/ConsistencyValidator.swift` | Stateless value type. Cross-eye sign mismatch + per-eye sign disagreement + per-eye SPH/CYL spread > 0.75 D → `.inconsistentAddPhoto` (under 5 photos) or `.inconsistentEscalate` (at 5). When 3+ photos and one disagrees with a consistent majority, drops that reading and surfaces it in `.consistent(droppedOutliers:)` so the operator sees every automatic exclusion |
