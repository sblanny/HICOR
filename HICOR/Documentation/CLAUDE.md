...
| `Views/AnalysisPlaceholderView.swift` | Spinner screen; runs OCR pipeline + ConsistencyValidator; presents add-photo / escalate-to-team-leader / error alerts; on `.consistent` builds in-memory `PatientRefraction` and navigates to `PrescriptionAnalysisView` forwarding `droppedOutliers`. Does NOT save — persistence happens on Save & Return |
...
| `Services/OCR/ConsistencyValidator.swift` | Stateless value type. Cross-eye sign mismatch + per-eye sign disagreement + pairwise AVG-to-AVG agreement check between printouts using `Constants.sphAgreementThreshold` (1.00 D), `Constants.cylAgreementThreshold` (0.50 D), and `AxisMath.toleranceForCyl` sliding scale (per `MIKE_RX_PROCEDURE.md` §1, §2, §4). Disagreement → `.inconsistentAddPhoto` (under max printouts) or `.inconsistentEscalate` (at max). When 3+ photos and one printout's AVG clearly disagrees with the majority, strips that printout's eye section and surfaces the dropped readings in `.consistent(droppedOutliers:)` so the operator sees every automatic exclusion |
| `Utilities/Constants.swift` | App-wide constants and shared enums (`Eye`, `MachineType`) |
...
- **2–5 printouts per patient (non-negotiable).** Per `MIKE_RX_PROCEDURE.md` and team-leader direction, every patient is scanned on the autorefractor 2–5 times. Cross-printout consistency validation in `ConsistencyValidator` is a **clinical safety gate**, not an OCR workaround. If 5 printouts still disagree, the app escalates to the team leader rather than averaging noise into a prescription.
- **Every automatic exclusion is visible.** When `ConsistencyValidator` drops an outlier reading (majority agrees, one disagrees), the dropped reading comes back in `Result.consistent(droppedOutliers:)` and `PrescriptionAnalysisView` renders it in a red warning banner. Silent drops are forbidden — the app makes clinical decisions, but never hides them from the operator.
